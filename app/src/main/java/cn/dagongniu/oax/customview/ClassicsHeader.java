...
//    public static String REFRESH_HEADER_UPDATE = "'Last update' M-d HH:mm";
    ...
    public void setLanguage(List<String> list, Context context) {
        ...
        REFRESH_HEADER_PULLING = list.get(0);

        REFRESH_HEADER_REFRESHING = list.get(1);

        REFRESH_HEADER_LOADING = list.get(2);

        REFRESH_HEADER_RELEASE = list.get(3);

        REFRESH_HEADER_FINISH = list.get(4);

        REFRESH_HEADER_FAILED = list.get(5);

        REFRESH_HEADER_UPDATE = list.get(6);

        REFRESH_HEADER_SECONDARY = list.get(7);
...
//    }
    ...
    public ClassicsHeader setLastUpdateTime(Date time) {
        final View thisView = this;
        mLastTime = time;
        final String format = mLastUpdateFormat.format(time);
