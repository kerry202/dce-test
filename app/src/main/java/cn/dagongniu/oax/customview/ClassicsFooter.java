...
public class ClassicsFooter extends InternalClassics<com.scwang.smartrefresh.layout.footer.ClassicsFooter> implements RefreshFooter {
     ...
     * @param context
     */
    public void setLanguage(List<String> list, Context context) {

        REFRESH_FOOTER_PULLING = list.get(0);

        REFRESH_FOOTER_RELEASE = list.get(1);

        REFRESH_FOOTER_LOADING = list.get(2);

        REFRESH_FOOTER_REFRESHING = list.get(3);

        REFRESH_FOOTER_FINISH = list.get(4);

        REFRESH_FOOTER_FAILED = list.get(5);

        REFRESH_FOOTER_NOTHING = list.get(6);
