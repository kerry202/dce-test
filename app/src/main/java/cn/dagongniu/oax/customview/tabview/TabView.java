...
public class TabView extends RelativeLayout {
    ...
    private void initTabChildView() {
        ...
        //加载交易页面
        if (mTabViewChildList.size() >= 1 && !mFragments[1].isAdded()) {
            mSupportFragmentManager.beginTransaction().add(com.ycl.tabview.library.R.id.tabview_fragment_container, mFragments[1]).hide(mFragments[1]).commit();
        }
    ...
    public void checkFragment(int index) {
        Log.d("sdsadasdads", "sdadsadadddsadsa");
        reSetAll();
        TabViewChild t1 = mTabViewChildList.get(index);
        t1.imageView.setImageResource(t1.getImageViewSelIcon());
        t1.textView.setText(t1.getTextViewText());
        t1.textView.setTextColor(mTextViewSelColor);
        this.index = index;
        showOrHide();
    }
    ...
    private void showOrHide() {
        if (currentTabIndex != index) {
            ...
            if (!mFragments[index].isAdded()) {
                trx.add(com.ycl.tabview.library.R.id.tabview_fragment_container, mFragments[index]);
            }
