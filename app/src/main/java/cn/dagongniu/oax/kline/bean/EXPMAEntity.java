...
public class EXPMAEntity {
    ...
    public EXPMAEntity(ArrayList<TradingInfoBean.KlineListBean> kLineBeens, int n) {
        ...
        if (kLineBeens != null && kLineBeens.size() > 0) {
            ...
            for (int i = 0; i < kLineBeens.size(); i++) {
                if (i == 0) {
                    ema = kLineBeens.get(i).getClose().floatValue();
                } else {
                    ema = (yz * kLineBeens.get(i).getClose().floatValue()) + ((1 - yz) * ema);
