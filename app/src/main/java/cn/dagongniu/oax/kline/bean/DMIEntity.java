...
public class DMIEntity {
     ...
     */
    public DMIEntity(List<TradingInfoBean.KlineListBean> kLineBeens, int dIn, int aDXn, int aDXRn, boolean isDIMA) {
        float tr = 0.0f;
        float tr1 = 0.0f;
        float tr2 = 0.0f;
        float tr3 = 0.0f;
        ...
        TradingInfoBean.KlineListBean lineBean;
        ...
        for (int i = 0; i < kLineBeens.size(); i++) {
            ...
            if (i == 0) {
                ...
                nowC = Float.valueOf(lineBean.getClose().toPlainString());
                tr = Float.valueOf(lineBean.getHigh().toPlainString()) - Float.valueOf(lineBean.getLow().toPlainString());
            } else {
                float high = Float.valueOf(lineBean.getHigh().toPlainString());
                float low = Float.valueOf(lineBean.getLow().toPlainString());

                dmH = high - Float.valueOf(kLineBeens.get(i - 1).getHigh().toPlainString());
                dmL = Float.valueOf(kLineBeens.get(i - 1).getLow().toPlainString()) - low;
