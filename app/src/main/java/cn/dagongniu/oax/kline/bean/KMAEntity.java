...
public class KMAEntity {
     ...
     * @param n         几日均值
     */
    public KMAEntity(ArrayList<TradingInfoBean.KlineListBean> kLineBeen, int n) {
     ...
     */
    public KMAEntity(ArrayList<TradingInfoBean.KlineListBean> kLineBeen, int n, float defult) {
    ...
    private static float getSum(Integer a, Integer b, ArrayList<TradingInfoBean.KlineListBean> datas) {
        ...
        for (int i = a; i <= b; i++) {
            sum = sum.add(datas.get(i).getClose());
        }
//        return sum.floatValue();
        return Float.valueOf(sum.toPlainString());
    }
