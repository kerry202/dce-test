 ...
 * 发送短信验证码 module
 */
public class SendSmsModule extends BaseModule<String, HttpBaseBean> {

    public SendSmsModule(Activity activity) {
        super(activity);
    }
    ...
    public void requestServerDataOne(final OnBaseDataListener<HttpBaseBean> onBaseDataListener, RequestState state, String... parm) {
        ...
        hashMap.put("type", type);                      //发短信 类型 暂定 1 注册 2登陆.......

        HttpUtils.getInstance().postLang(Http.user_sendSms, hashMap, activity, new OnBaseDataListener<String>() {
            @Override
