 ...
 * 登陆校验密码 module
 */
public class CheckLoginPasswordModule extends BaseModule<String, HttpBaseBean> {

    public CheckLoginPasswordModule(Activity activity) {
        super(activity);
    }
    ...
    public void requestServerDataOne(final OnBaseDataListener<HttpBaseBean> onBaseDataListener, RequestState state, String... parm) {
        ...
        hashMap.put("username", username);                    //用户名
        hashMap.put("password", password);                    //密码

        HttpUtils.getInstance().postLang(Http.USER_CHECKLOGINPASSWORD, hashMap, activity, new OnBaseDataListener<String>() {
            @Override
