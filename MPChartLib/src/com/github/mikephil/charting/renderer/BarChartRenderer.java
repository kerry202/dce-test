    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
                ...
                int i = j / 4;
                if (i > 0) {
                    boolean isFromKline = dataSet.getEntryForIndex(i).isFromKline();
                    int green = dataSet.getColor(0);
                    int red = dataSet.getColor(1);
                    if (isFromKline) {
                        float close = dataSet.getEntryForIndex(i).getClose();
                        float open = dataSet.getEntryForIndex(i).getOpen();
                        if (open > close)
                            mRenderPaint.setColor(red);
                        else if (open < close)
                            mRenderPaint.setColor(green);
                        else
                            mRenderPaint.setColor(green);
                    } else {
                        //目前在MACD中用到
                        if (dataSet.getEntryForIndex(i).getVal() > 0) {
                            mRenderPaint.setColor(green);
                        } else {
                            mRenderPaint.setColor(red);
                        }
