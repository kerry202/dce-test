...
//                    ", content width: " + mViewPortHandler.contentWidth());
    ...
    protected float[] getMarkerPosition(Entry e, Highlight highlight) {
        ...
        if (this instanceof BarChart) {
            ...
            if (e instanceof CandleEntry) {
                yPos = ((CandleEntry) e).getClose() * mAnimator.getPhaseY();
            } else {
                yPos *= mAnimator.getPhaseY();
            }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        requestDisallowInterceptTouchEvent(true);//防止与ScrollView冲突，不会返回MotionEvent.ACTION_CANCEL
//        Log.d("onTouchEvent ", "ev == " + event.getAction());
        if (mChartTouchListener == null || mData == null)
            return false;

        // check if touch gestures are enabled
        if (!mTouchEnabled) {
//            Log.d("onTouchEvent ", "mTouchEnabled == " + mTouchEnabled);
            return false;
        } else {
//            Log.d("onTouchEvent ", "mTouchEnabled == " + mTouchEnabled);
